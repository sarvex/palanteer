...
def measure_event_size_and_timing():
    ...
    refO_build_time_sec, refO_program_size, refO_exec_time_sec    = _evaluate_perf_program("", ["-DUSE_PL=1", "-O2"])
    ref1O_build_time_sec, ref1O_program_size, ref1O_exec_time_sec = _evaluate_perf_program("", ["-DUSE_PL=1", "-DPL_IMPL_CONTEXT_SWITCH=0", "-O2"])
    ref2O_build_time_sec, ref2O_program_size, ref2O_exec_time_sec = _evaluate_perf_program("", ["-DUSE_PL=1", "-DPL_NOCONTROL=1", "-O2"])
    evtO_build_time_sec, evtO_program_size, evtO_exec_time_sec    = _evaluate_perf_program("plVar(abcdefghij);", ["-DUSE_PL=1", "-O2"])

    # Optimized assert measures
    assertO_build_time_sec, assertO_program_size, assertO_exec_time_sec    = _evaluate_perf_program("plAssert(abcdefghij++);", ["-DUSE_PL=1", "-O2"], 1)
    assertEO_build_time_sec, assertEO_program_size, assertEO_exec_time_sec = _evaluate_perf_program("plAssert(abcdefghij++, abcdefghij, i);", ["-DUSE_PL=1", "-O2"], 1)
    refSO_build_time_sec, refSO_program_size, refSO_exec_time_sec          = _evaluate_perf_program("", ["-DUSE_PL=1", "-DPL_SIMPLE_ASSERT=1", "-O2"], 1)
    assertSO_build_time_sec, assertSO_program_size, assertSO_exec_time_sec = _evaluate_perf_program("plAssert(abcdefghij++, abcdefghij, i);", ["-DUSE_PL=1", "-DPL_SIMPLE_ASSERT=1", "-O2"], 1)

    # Debug event measures (for build timings only)
    debugOpt = "-Od" if sys.platform=="win32" else "-O0"
    refg_build_time_sec, refg_program_size, refg_exec_time_sec   = _evaluate_perf_program("", ["-DUSE_PL=1", debugOpt])
    evtg_build_time_sec, small_program_size, small_exec_time_sec = _evaluate_perf_program("plVar(abcdefghij);", ["-DUSE_PL=1", debugOpt])
    ...
    KPI("Assert code size", "%.1f bytes/assert" % ((assertO_program_size-refO_program_size)/1024.))
    KPI("Assert+2 integers code size", "%.1f bytes/assert" % ((assertEO_program_size-refO_program_size)/1024.))
    KPI("Assert (simple) code size", "%.1f bytes/assert" % ((assertSO_program_size-refSO_program_size)/1024.))
